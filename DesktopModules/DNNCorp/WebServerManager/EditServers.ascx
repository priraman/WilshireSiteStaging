...
<div class="dnnForm dnnEditServers dnnClear" ID="dnnEditServers">
	...
	<h2 ID="dnnSitePanel-EditWebRequestAdapter" class="dnnFormSectionHead"><a href="" class="dnnSectionExpanded"><%=LocalizeString("WebRequestAdapter")%></a></h2>
	<fieldset>
	    <div class="dnnFormItem">
            <dnn:Label ID="plWebRequestAdapter" runat="server" ControlName="cbWebRequestAdapter" />
            <dnn:dnncombobox ID="cbWebRequestAdapter" runat="server"></dnn:dnncombobox>
        </div>
        <ul class="dnnActions dnnClear">
			<li><asp:LinkButton ID="cmdSaveAdapter" runat="server" ResourceKey="grd.Save" CssClass="dnnPrimaryAction" /></li>
		</ul>
    </fieldset>
	...
	<fieldset>
		<div class="dnnFormItem">
            ...
            <dnn:DnnComboBox ID="CachingProviders" runat="server" />
		</div>
		<div class="dnnFormItem">
		...
		<div class="dnnFormItem">
			<dnn:label ID="chkUseSSLLabel" ControlName="chkUseSSL" runat="server" />
	...
	<fieldset>
        <div class="filterPanel">
            <span><%=LocalizeString("Filter")%></span>
            <ul>
                <li><a href="<%=DotNetNuke.Common.Globals.NavigateURL(string.Empty, "type=1") %>" class="<%=Request.QueryString["type"] == "1" ? "selected" : "" %>"><%=LocalizeString("Enabled")%></a></li>
                <li><a href="<%=DotNetNuke.Common.Globals.NavigateURL(string.Empty, "type=2") %>" class="<%=Request.QueryString["type"] == "2" ? "selected" : "" %>"><%=LocalizeString("Disabled")%></a></li>
                <li><a href="<%=DotNetNuke.Common.Globals.NavigateURL(string.Empty) %>" class="<%=Request.QueryString["type"] != "1" && Request.QueryString["type"] != "2" ? "selected" : "" %>"><%=LocalizeString("All")%></a></li>
            </ul>
        </div>
		...
		<asp:DataList ID="lstServers" runat="server" RepeatLayout="Flow" RepeatDirection="Horizontal" DataKeyField="ServerID" OnItemDataBound="OnServersItemDataBound" OnEditCommand="OnServersEditCommand" OnCancelCommand="OnServersCancelCommand" OnUpdateCommand="OnServersUpdateCommand" OnDeleteCommand="OnServersDeleteCommand" >
			...
			<ItemTemplate>
				<fieldset class="mwsEditServers dnnClear">
					<div class="dnnLeft"><dnn:DnnImage ID="WS" runat="server" IconKey="Webserver" IconSize="120X120" AlternateText='<%#Eval("ServerName")%>' CssClass="dnnServerImage"></dnn:DnnImage></div>
					<div class="dnnLeft dnnServerInfo">
						<div class="dnnFormItem">
							<dnn:Label runat="server" ResourceKey="grd.Server" AssociatedControlID="lblServerName" />
							<asp:Label ID="lblServerName" runat="server"><%#Eval("ServerName")%></asp:Label>
						</div>
						<div class="dnnFormItem">
							<dnn:Label runat="server" ResourceKey="grd.IISAppName" AssociatedControlID="lblIisAppName" />
							<asp:Label ID="lblIisAppName" runat="server"><%#Eval("IISAppName")%></asp:Label>
						</div>
						<div class="dnnFormItem">
							<dnn:Label runat="server" ResourceKey="grd.URL" AssociatedControlID="lblUrl" />
							<asp:Hyperlink ID="lblUrl" runat="server" NavigateUrl='<%#  GetUrl(Eval("Url").ToString()) %>'><%#Eval("Url")%></asp:Hyperlink>
						</div>
						<div class="dnnFormItem">
							<dnn:Label runat="server" ResourceKey="grd.ServerGroup" AssociatedControlID="groupLabel" />
						    <asp:Label ID="groupLabel" runat="server"><%#Eval("ServerGroup")%></asp:Label>
						</div>
						<div class="dnnFormItem">
							<dnn:Label runat="server" ResourceKey="grd.CreatedDate" AssociatedControlID="lblCreatedDate" />
							<asp:Label ID="lblCreatedDate" runat="server"><%#Eval("CreatedDate")%></asp:Label>
						</div>
						<div class="dnnFormItem">
							<dnn:Label runat="server" ResourceKey="grd.LastActivityDate" AssociatedControlID="lblLastActivityDate" />
							<asp:Label ID="lblLastActivityDate" runat="server"><%#Eval("LastActivityDate")%></asp:Label>
						</div>
						<div class="dnnFormItem">
							<dnn:Label runat="server" ResourceKey="grd.Enabled" AssociatedControlID="lblEnabled" />
			...
			<EditItemTemplate>
				<fieldset>
					...
					<div class="dnnFormItem">
						<dnn:Label runat="server" ResourceKey="grd.URL" AssociatedControlID="txtAlias" />						
                        <asp:TextBox ID="txtAlias" runat="server" />
                        <asp:CustomValidator runat="server" ControlToValidate="txtAlias" EnableClientScript="false"
                            CssClass="dnnFormMessage dnnFormError" resourcekey="InvalidUrl.ErrorMessage" 
                            OnServerValidate="ValidateUrl" Display="Dynamic"></asp:CustomValidator>
					</div>
					<div class="dnnFormItem">
						<dnn:Label runat="server" ResourceKey="grd.ServerGroup" AssociatedControlID="groupTextBox" />
					...
					<div class="dnnFormItem">
						<dnn:Label runat="server" ResourceKey="grd.Enabled" AssociatedControlID="chkEnabled" />
	...
	<fieldset>
		...
		<div class="dnnFormItem" ID="rowItems" runat="server" visible="false">
		    ...
		    <dnn:DnnComboBox ID="cboItems" runat="server" AutoPostBack="true" />
		</div>
		...
		<div class="dnnFormItem" visible="false" runat="server" ID="rowValue">
            ...
            <div class="dnnLeft" style="width: 50%; margin-top: 5px">
    ...
    <h2 ID="dnnSitePanel-SSLOffload" class="dnnFormSectionHead"><a href="" class="dnnSectionExpanded"><%=LocalizeString("SSLOffload")%></a></h2>
	<fieldset>
		<div class="dnnFormItem">
            <dnn:label id="plSSLOffload" runat="server" controlname="txtSSloffload" suffix=":" />
            <asp:TextBox runat="server" ID="txtSSloffload" autocomplete="off" />
		</div>
        <ul class="dnnActions dnnClear">
			<li><asp:LinkButton ID="SaveSSLOffload" runat="server" ResourceKey="grd.Save" CssClass="dnnPrimaryAction" /></li>
		</ul>
	</fieldset>
</div>
