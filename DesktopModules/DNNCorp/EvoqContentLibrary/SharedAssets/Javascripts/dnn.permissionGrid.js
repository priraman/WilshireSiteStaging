...
(function ($) {
    ...
    dnn.controls.PermissionGrid.prototype = {
        ...
        _buildGrid: function () {
            var gridContainer, i, rolesTable, rolePermission, usersTable, j, userPermission;

            gridContainer = this._gridContainer = $('<div class="dnnGrid dnnPermissionsGrid dnnForm"></div>');

            rolesTable = $('<table class="dnnPermissionsGrid rolesGrid" cellspacing="0" cellpadding="2" style="border-collapse:collapse;"><tbody></tbody></table>');

            this._buildGridHeader(rolesTable.find('tbody'), this.data.permissionDefinitions, 'roles');

            for (i = 0; i < this.data.rolePermissions.length; i++) {
                rolePermission = this.data.rolePermissions[i];
                this._buildGridRow(rolesTable.find('tbody'), rolePermission, 'roles');
            }
            gridContainer.append(rolesTable);

            //create users permission table
            usersTable = $('<table class="dnnPermissionsGrid usersGrid" cellspacing="0" cellpadding="2" style="border-collapse:collapse;"><tbody></tbody></table>');

            this._buildGridHeader(usersTable.find('tbody'), this.data.permissionDefinitions, 'users');

            for (j = 0; j < this.data.userPermissions.length; j++) {
                userPermission = this.data.userPermissions[j];
                this._buildGridRow(usersTable.find('tbody'), userPermission, 'users');
            }
            gridContainer.append(usersTable);
